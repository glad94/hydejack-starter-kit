...
where indices *i* refer to consecutive events alternating between players *p* and *q*, &Delta xT &theta the change in xT per event and *j* the index denoting each event sequence as a whole. 
